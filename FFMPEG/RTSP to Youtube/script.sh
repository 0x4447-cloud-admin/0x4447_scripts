...
if [ $? == 0 ]; then
	...
	#
	#	1.	Since OS was found, set OSFound to 1.
	#
	OSFound=1;

	#
	#	2.	Check for installation of  ffmpeg.
	#
	dpkg -s ffmpeg 2>&1 | head -1 | grep "not installed" 1>/dev/null

	#
	#	3.	If ffmpeg is not installed, give ERROR,instructions, and exit script.
	#
	if [ $? == 0 ]; then
...
#
#	10.	Check if any OS is found, if not ERROR and exit.
#
if [ "$OSFound" == 0 ]; then
...
#
#	11.	Ask if user has saved information from previous use.
#
...
#
#	13.	Check if there was any input, if not exit.
#
if [ -z "$SavedInfoAns" ]; then
...
#
#	14.	Change the answer to Upper Case.
#
SavedInfo=$(echo "$SavedInfoAns" | awk '{print toupper($0)}')

#
#	15.	Check if the answer was Y or Yes.
#
if [ "$SavedInfo" == "Y" ] || [ "$SavedInfo" == "YES" ]; then

	#
	#	1.	Ask for path of saved file.
	#
	...
	#
	#	3.	Check if there was any input, if not request info.
	#
	if [ -z "$SavedPath" ]; then
...
#
#	16.	Check if SaveFile variable is 1 to pull data.
#
if [ $SavedFile == "1" ]; then
...
#
#	17.	Check if SavedFile is 0, meaning data to be requested.
#
if [ "$SavedFile" == "0" ]; then
	...
	#
	#	10.	Ask if user wants to save information for future use.
	#
	...
	#
	#	12.	Check if there was any input, if yes make it upper case.
	#
	if [ ! -z "$SaveInfoAns" ]; then
	...
	#
	#	13.	Check if user entered Y or YES.
	#
	if [ "$SaveInfo" == "Y" ] || [ "$SaveInfo" == "YES" ]; then

		#
		#	1.	Ask user for path of where file is to be saved.
		#
		...
		#
		#	3.	Check if there was any input, if not continue without saving..
		#
		if [ -z "$SaveFile" ]; then
			...
			#
			#	1.	Create file if a path was entered.
			#
			touch "$SaveFile"
		...
		#
		#	4.	Check if file exists, if not continue without saving.
		#
		if [ ! -f "$SaveFile" ]; then
			...
			#
			#	1.	Input entries to file provided.
			#
		else
...
#
#	18.	Assign variable the flags for the FFMPEG Input.
#
FFMPEG_INPUT="-rtsp_transport tcp -thread_queue_size 512 -i ${RTSPURL}"

#
#	19.	Assign variable the flags for the FFMPEG Output.
#
...
#
#	20.	Create variable that will run the whole command without any visible errors.
#
COMMAND="ffmpeg -hide_banner -nostdin -nostats ${FFMPEG_INPUT} ${FFMPEG_OUTPUT}"

#
#	21.	Check if a stream is already running.
#
ps -ef | grep -i ffmpeg | grep -v grep | grep -i rtmp 1>/dev/null 2>&1;

#
#	22.	Check if a stream is already running, if Yes then kill.
#
if [ $? == "0" ]; then
...
#
#	23.	Inform User streaming has started.
#
echo "Streaming started..."

#
#	24.	Run the COMMAND variable.
#
