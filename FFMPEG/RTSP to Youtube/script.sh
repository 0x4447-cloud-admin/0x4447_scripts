...
#
#	4.	If ffmpeg doesn't exist, show error and exit script with exit value of 1.
#
if [ $? == 1 ]; then
...
if [ -f "$SavedPath" ]; then
    ...
    if [ -z "$RTSPURL" ] || [ -z "$YoutubeURL" ] || [ -z "$YoutubeKey" ]; then
        ...
        #
        #	1.	Set ContinueScript to continue with requesting the information from user.
        #
        ContinueScript=1;
...
#
#	6.	Check if Saved credentials doesn't exist or if it didn't have proper information.
#
if [ ! -f $SavedPath ] || [ $ContinueScript == "1" ]; then
    ...
    #
    #	10.	Create empty file in the directory provided in SavedPath.
    #
    touch "$SavedPath"
    ...
    if [ ! -f "$SavedPath" ]; then
        ...
        #
        #	1.	Set ContinueScript back to 0 so it doesn't try to input the data.
        #
        ContinueScript=0;
    fi;
    
    #
    #	11.	Input entries to file provided.
    #
    ...
    #
    #	12.	End of if for getting the required information.
    #
fi;
...
#
#	9.	Create variable that will run the whole command without any visible errors.
#
...
#
#	11.	Check if a stream is already running, if Yes then exit script with value 1.
#
if [ $? == "0" ]; then
